...
from app.publix.spider import run_publix_batch

logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")

DEFAULT_OUTPUT = Path(__file__).with_name("spider_results") / "publix_products.jsonl"
...
def main(argv: list[str] | None = None):
    ...
    print(f"Starting Publix batch search for: {', '.join(queries)}")
    print(f"Store: {args.location_id or 'default'}")

    results = run_publix_batch(
        queries=queries,
